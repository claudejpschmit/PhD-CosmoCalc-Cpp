...
ClassEngine::ClassEngine(const ClassParams& pars): cl(0),dofree(true){
    ...
    for (size_t i=0;i<pars.size();i++){
        ...
        // THE COMMENTED CODE IS UNECESSARY FIR PK!
        //identify lmax ----CLAUDE unnecessary for Pk.
...
double ClassEngine::return_Pkz(double k, double z)
{
   
    double pk_tot;
    pk_tot = 0;
    double * pk_ic = NULL;
    spectra_pk_at_k_and_z(&ba, &pm, &sp, k, z, &pk_tot, pk_ic);
    return pk_tot;
}
