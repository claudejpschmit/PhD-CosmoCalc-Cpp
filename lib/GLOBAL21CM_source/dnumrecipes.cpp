...
double rf(const double x, const double y, const double z)
{
  ...
  if (fmin(fmin(x,y),z) < 0.0 || fmin(fmin(x+y,x+z),y+z) < TINY1 ||
      fmax(fmax(x,y),z) > BIG)
			 ...
			 void (*)(double, double [], double [])))
{
  ...
  *nbad = 0;
  ...
  for (nstp=1;nstp<=MAXSTP;nstp++) {
    (*derivs)(x,y,dydx);
    for (i=1;i<=nvar;i++) {
      yscal[i]=fabs(y[i])+fabs(dydx[i]*h)+TINY1;
	    ...
	    double *hnext, void (*derivs)(double, double [], double []))
{
  ...
  int i,iq,k,kk,km(0);
  double eps1,errmax(TINY1),fact,h,red(SAFE2),scale(1.0),work,wrkmin,xest;
